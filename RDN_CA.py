...
class RDB(nn.Module):
    ...
    def forward(self, x):
        return x + ChannelAttention(self.lff(self.layers(x)))  # local residual learning


class RDN_CA(nn.Module):
    def __init__(self, blocksize = 32, subrate = 0.1, num_channels = 1, num_features = 32, growth_rate = 32, num_blocks = 16, num_layers = 8,reduction = 8):
